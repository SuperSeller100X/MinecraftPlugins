 ...
 * {@code {name}} syntax and are substituted before MiniMessage parsing. Every
 * user-facing string of SwiftTPA lives in messages.yml so servers can fully
 * restyle or translate the plugin.
 */
public final class Messages {
    ...
    public Messages(SwiftTPAPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void sendHelp(CommandSender sender, String section) {
        ...
        if (lines.isEmpty()) {
            sender.sendMessage(deserialize("<gold>" + plugin.getName() + "</gold> <gray>run /stpa help</gray>"));
            return;
        }
