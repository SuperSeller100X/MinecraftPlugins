...
public final class Messages {
    ...
    private String prefix = "<gradient:#ff5f6d:#ffc371><b>CombatTag</b></gradient> <dark_gray>»</dark_gray> ";
    ...
    public void send(CommandSender sender, String key, String... placeholderPairs) {
        ...
        try {
            ...
            try {
                sender.sendMessage(Component.text("§c[CombatTag] " + key));
    ...
    /**
     * Resolves a list of message lines (used for GUI lore).
     *
     * @param key message key holding a string list
     * @param placeholders placeholder replacements
     * @return the formatted components, never null
     */
    public java.util.List<Component> getList(String key, Map<String, String> placeholders) {
        java.util.List<Component> out = new java.util.ArrayList<>();
        if (config == null) {
            return out;
        }
        java.util.List<String> raw = config.getStringList(key);
        for (String line : raw) {
            String text = line == null ? "" : line;
            if (placeholders != null) {
                for (Map.Entry<String, String> entry : placeholders.entrySet()) {
                    String v = entry.getValue() != null ? entry.getValue() : "";
                    text = text.replace("<" + entry.getKey() + ">", v);
                    text = text.replace("{" + entry.getKey() + "}", v);
                }
            }
            text = convertLegacyToMiniMessage(text);
            try {
                out.add(miniMessage.deserialize(text));
            } catch (Throwable t) {
