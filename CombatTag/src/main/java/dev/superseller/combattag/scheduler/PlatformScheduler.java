package dev.superseller.combattag.scheduler;
...
public final class PlatformScheduler {
    ...
    /**
     * Schedules a repeating global task, Folia-aware.
     *
     * @param delayTicks initial delay in ticks (minimum 1)
     * @param periodTicks repeat period in ticks (minimum 1)
     * @param task task body
     * @return a cancel handle, never null
     */
    public static Runnable runTimer(long delayTicks, long periodTicks, Runnable task) {
        long delay = Math.max(1L, delayTicks);
        long period = Math.max(1L, periodTicks);
        if (globalSchedulerGet != null) {
            try {
                Object scheduler = globalSchedulerGet.invoke(null);
                ...
                Object handle = runAtFixedRate.invoke(scheduler, plugin,
                        (Consumer<Object>) unused -> task.run(), delay, period);
                return () -> {
                    try {
                ...
                // Fall through to Bukkit scheduling
            }
        }
        try {
            ...
            logger.warning("Could not schedule repeating task: " + e.getMessage());
            return () -> { };
        }
    }
